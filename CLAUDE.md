...
- **Dual imports everywhere**: every cross-module import is wrapped `try: from . import x / except ImportError: import x` so the plugin works both as a package (in Hermes) and as flat modules (in tests/CI). This now covers nine modules (`adapter`, `mesh_tools`, `schemas`, `telemetry_db`, `chunking`, `mock_interface`, `node_freshness`, `transport`, `ack_state`) — keep the pattern when adding modules.
